...
pub fn bezier(m_x1: f32, m_y1: f32, m_x2: f32, m_y2: f32) -> Box<dyn Fn(f32) -> f32> {
    assert!(
        (0.0..=1.0).contains(&m_x1) && (0.0..=1.0).contains(&m_x2),
    ...
    // Return a closure that calculates y(t) for any x input
    Box::new(move |x: f32| {
        if x == 0.0 || x == 1.0 {
